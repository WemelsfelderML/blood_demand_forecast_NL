...
# echo "rmarkdown::render('forecast_script_FIN_wold.Rmd', clean=TRUE,output_format='html_document',output_file='results/forecast_script_FIN_wold_20201022.html')" | R --slave
...
plat.2004<- red.2004[order(plat.2004$`year(date)`,plat.2004$`month(date)`),]
...
Wow, there something in here I need to check...
...
#beginning <- head(tail(monthly$date, ((rw_years+1) * 12 + 1)), 1) # Define beginning of (monthly) time here THIS IS FOR USING ONLY LAST FOR YEARS OF DATA
#beginning <- tail(head(monthly$date, ((rw_years+1) * 12 + 1)), 1)
beginning <- tail(head(monthly$date, ((rw_years) * 12 + 1)), 1) #When using full data start after 3 years of data beginning
...
It would be nice to add the NL data on the same plot!
